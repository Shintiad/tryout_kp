...
class Template
{
    ...
    public static function message($message = '', $type = 'information')
    {
        ...
        if (empty($message) && !self::$ignore_session && class_exists('CI_Session', false)) {
            $message = self::$ci->session->flashdata('message');

            if (!empty($message)) {
                // Check if message is an array
                if (is_array($message)) {
                    // You can choose to implode the array or handle it differently
                    $message = implode(', ', $message); // Converts array to string
                }
    
                // Split out the message parts
                $temp_message = explode('::', $message);
                if (count($temp_message) == 2) {
                    $type = $temp_message[0];
                    $message = $temp_message[1];
                } else {
                    // Handle cases where the message format is not as expected
                    $type = 'information'; // or set a default type
