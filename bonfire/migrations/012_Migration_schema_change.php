...
class Migration_Migration_schema_change extends Migration
{
	...
	private $schema_table = 'schema_version_new';
	...
	public function up()
	{
		...
		if (!$this->db->field_exists($this->new_key, $this->table_name)) {
			...
			$this->dbforge->create_table($this->schema_table);
			...
			if (!empty($permissionRecords)) {
				$this->db->insert_batch($this->schema_table, $permissionRecords);
			}

			// Backup the table.
			$this->dbforge->rename_table($this->schema_table, $this->table_name);
		}
	}
