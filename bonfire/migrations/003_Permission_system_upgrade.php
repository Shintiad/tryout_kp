...
class Migration_Permission_system_upgrade extends Migration
{
	...
	public function up()
	{
		...
		if (!$this->db->field_exists('Site_Signin_Offline', $this->permissions_old_table)) {
			$this->dbforge->add_column($this->permissions_old_table, $this->permissions_fields);

			$this->db->where('role_id', $this->admin_role_id)
				->update($this->permissions_old_table, $this->permissions_data);
		}

		// Rename Site_Statistics_View to Site_Reports_View
		if ($this->db->field_exists('Site_Statistics_View', $this->permissions_old_table)) {
			$this->dbforge->modify_column($this->permissions_old_table, $this->permissions_modify_fields);
		}

		// Remove Site.Appearance.View
		if ($this->db->field_exists('Site_Appearance_View', $this->permissions_old_table)) {
			foreach ($this->permissions_drop_fields as $column_name => $column_def) {
				$this->dbforge->drop_column($this->permissions_old_table, $column_name);
			}
		}
		...
		$permission_query = $this->db->get($this->permissions_old_table);
