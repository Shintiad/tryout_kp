...
if (!function_exists('form_dropdown_lte')) {
	/**
	 * Returns a properly templated dropdown field.
	 *
	 * @param string $data    The element name or an array of key/value pairs of
	 ...
	 * @return string The formatted input element, label tag and wrapping divs.
	 */
	function form_dropdown_lte($data, $options = array(), $selected = array(), $label = '', $extra = '', $tooltip = '')
	{
		if (!is_array($data)) {
			$data = array('name' => $data);
		}
...
<div class='form-group{$error}'>
	<label class='' for='{$data['id']}'>{$label}</label>
	<select {$output} {$extra} style='width: 100%;'>
		{$options_vals}
	</select>
	{$tooltip}
</div>";
