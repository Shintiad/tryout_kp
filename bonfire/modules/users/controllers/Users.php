...
class Users extends Front_Controller
{
	...
	public function login()
	{
		...
		if (
			...
			if ($user) {
				// Jika role_id = 1, redirect ke select_school
				if ($user->role_id == 1) {
					$this->session->userdata();
					Template::redirect('users/select_school');
				} else {
					// Ambil sekolah_id dari user dan simpan sebagai id_sekolah di session
					$this->session->set_userdata('id_sekolah', $user->sekolah_id);
					$this->session->set_userdata('sekolah_id', $user->sekolah_id);
				}
			}
			...
			if (
				$this->settings_lib->item('auth.do_login_redirect')
	...
	public function simpanSekolah()
	{
		...
		// Redirect ke halaman admin content
	...
	public function get_sub_kelas_options()
	{
		$kelas_id = $this->input->get('kelas_id');
		$this->load->model('user_model');
		$sub_kelas_options = $this->user_model->get_sub_kelas_by_kelas_id($kelas_id);
		echo json_encode($sub_kelas_options);
	}
	...
	public function get_guru_options()
	{
		...
		$this->load->model('User_model');
		$guru_options = $this->User_model->get_guru_options($sekolah_id);
		echo json_encode($guru_options);
	}
	...
	public function register()
	{
		...
		// Load sub_kelas dan sekolah options
		$this->load->model('user_model'); // pastikan model user_model sudah benar
		$role_options = $this->user_model->get_role_options();
		$kelas_options = $this->user_model->get_kelas_options();
		$sub_kelas_options = $this->user_model->get_sub_kelas_options();
		$sekolah_options = $this->user_model->get_sekolah_options();

		Template::set('role_options', $role_options);
		Template::set('kelas_options', $kelas_options);
		Template::set('sub_kelas_options', $sub_kelas_options);
		Template::set('sekolah_options', $sekolah_options);

		if (isset($_POST['register'])) {
			$kelas_id = $this->input->post('kelas_id');
			$kelas_options = $this->user_model->get_kelas_options();

			if (isset($kelas_options[$kelas_id])) {
				$_POST['program_id'] = $kelas_options[$kelas_id]['program_id'];
			}
			...
			if ($userId = $this->saveUser('insert', 0, $meta_fields)) {
				...
				$waiting_url = site_url("users/waiting/{$user_role}");
				redirect($waiting_url);
			}
	...
	public function waiting($role_id = null)
	{
		if (!$role_id) {
			Template::redirect('/');
		}
		...
		if ($role_id == '10') { // Guru
			$data['message'] = "Thank you for registering as a teacher ({$role_name}). Please wait for admin verification.";
			$data['title'] = 'Waiting for Admin Verification';
		} elseif ($role_id == '11') { // Siswa
			$data['message'] = "Thank you for registering as a student ({$role_name}). Please wait for teacher verification.";
			$data['title'] = 'Waiting for Teacher Verification';
		} else {
			$data['message'] = "Thank you for registering as {$role_name}. Your account is pending verification.";
			$data['title'] = 'Account Pending Verification';
		}

		Template::set('data', $data);
		Template::set_view('users/waiting');
		Template::render();
	}
			...
			if ($this->form_validation->run()) {
				...
				if ($activated) {
					...
					if ($this->emailer->send($data)) {
						...
						Template::set_message(lang('us_err_wait') . $this->emailer->error, 'error');
					}
	...
	private function saveUser($type = 'insert', $id = 0, $metaFields = array())
	{
		...
		$this->form_validation->set_rules('kelas_id', 'Kelas', 'trim|integer');
		$this->form_validation->set_rules('program_id', 'Program', 'trim|integer');

		$usernameRequired = '';
		if (
			$this->settings_lib->item('auth.login_type') == 'username'
	...
	public function select_school()
	{
		$sekolah_options = $this->user_model->get_sekolah_options();
		Template::set('sekolah_options', $sekolah_options);
		Template::set_view('users/select_school');
		Template::set('page_title', 'Select_School');
		Template::render();
	}
}
