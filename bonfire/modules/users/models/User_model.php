...
class User_model extends BF_Model
{
    ...
    protected $validation_rules = [
        ...
        [
            'field' => 'country',
            'label' => 'Country',
            'rules' => 'trim|required|in_list[Indonesia]',
        ],
        [
            'field' => 'nip',
            'label' => 'Nip',
        ...
        [
            'field' => 'telepon',
            'label' => 'Telepon',
            'rules' => 'trim|max_length[13]|numeric',
        ],
        [
            'field' => 'sub_kelas_id',
            'label' => 'Sub Kelas',
    ...
    {
        $this->db->where('id', $id);
        $query = $this->db->get('users'); // Use your table name here
    ...
    {
        $this->db->select('tb_kelas.id_kelas, tb_kelas.kelas, tb_program.id_program, tb_program.name as program');
        $this->db->from('tb_kelas');
        $this->db->join('tb_program', 'tb_kelas.program_id = tb_program.id_program', 'left');
        $this->db->order_by('tb_kelas.kelas', 'asc');
        $query = $this->db->get();
        $options = [];
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                $options[$row->id_kelas] = [
                    'text' => $row->kelas . ' ' . $row->program,
                    'program_id' => $row->id_program
                ];
            }
        }
        return $options;
    ...
    public function get_kelas_info($kelas_id)
    {
        $this->db->select('tb_kelas.id_kelas, tb_kelas.kelas, tb_program.id_program, tb_program.name as program');
        $this->db->from('tb_kelas');
        $this->db->join('tb_program', 'tb_kelas.program_id = tb_program.id_program', 'left');
        $this->db->where('tb_kelas.id_kelas', $kelas_id);
        $query = $this->db->get();
    ...
    public function get_sub_kelas_by_kelas_id($kelas_id)
    {
        $this->db->select('id_sub_kelas, sub_kelas');
        $this->db->from('tb_sub_kelas');
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                $options[$row->id_sub_kelas] = $row->sub_kelas;
            }
        }
        return $options;
    }
    ...
    public function get_sekolah_options()
    {
        $this->db->select('id_sekolah, name');
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result() as $row) {
                $options[$row->id_sekolah] = $row->name;
            }
        }
        return $options;
    }

    public function get_guru_options($sekolah_id)
    {
    ...
    public function insert($data = [])
    {
        ...
        if (empty($data['display_name'])) {
            if (
                $this->settings_lib->item('auth.use_usernames') == 1
                && ! empty($data['username'])
            ) {
        ...
        if ($data['sub_kelas_id'] === '') {
            $data['sub_kelas_id'] = null;
        }
    ...
    public function set_activation($user_id)
    {
        ...
        if ($this->emailer->send($data)) {
            ...
            $message .= lang('us_err_wait') . $this->emailer->error;
            $error = true;
        }
