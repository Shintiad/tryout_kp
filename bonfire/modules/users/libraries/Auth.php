...
class Auth
{
    ...
    public function login($login, $password, $remember = false)
    {
        ...
        if ($user->status == 'f') {
            if ($user->role_id == 10) {
                Template::set_message(lang('us_account_ver_guru'), 'error');
                return false;
            } else if ($user->role_id == 11) {
                Template::set_message(lang('us_account_ver_siswa'), 'error');
                return false;
            }
        }
    ...
    public function user()
    {
        ...
        if (
            sha1($this->ci->session->userdata('user_id') . $user->password_hash)
