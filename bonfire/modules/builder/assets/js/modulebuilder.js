...
bonfire.builder = {
	...
	prepTableDefaults: {
		...
		valueToPrep: '',
		/**
		 * Determines whether the function sets the module description in addition to
		 * returning the string (uses moduleDescriptionSelector to set the module description)
		 */
		...
		/** The selector for the field containing the module description */
		moduleDescriptionSelector: '#module_description',
	...
	prepTableName: function (prepOptions) {
		...
		if (prepSettings.setModuleDescription === true) {
			$(prepSettings.moduleDescriptionSelector).val('Modul ' + prepSettings.valueToPrep);
		}
 ...
 */
$('#module_name, #table_name').on('keyup', function () {
	bonfire.builder.prepTableName({ valueToPrep: this.value, setModuleDescription: true, setTableName: true });
});

$('#all_fields').on('keyup', '.view_field_label', function () {
	$('#view_field_name' + $(this).data('count')).val(this.value.replace(/[^A-Za-z0-9\\s]/g, '_').toLowerCase());
});