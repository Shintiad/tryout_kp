...
if ($db_required != '') {
	...
	$indexFind .= "\$records = \$this->{$module_name_lower}_model->find_all();
        Template::set('records', \$records);";
}

// If this is not the front controller, setup the toolbar title
if ($controller_name_lower != $module_name_lower) {
	$indexToolbarTitle = "Template::set('toolbar_title', lang('{$module_name_lower}_manage'));";
}
...
for ($counter = 1; 0 >= $counter; $counter++) {
...
if ($controller_name_lower != $module_name_lower) {
	$baseClass = 'App_Controller';
	$constructorRestrict = "\$this->auth->restrict(\$this->permissionView);";
	$subNav = "Template::set_block('sub_nav', '{$controller_name_lower}/_sub_nav');";
	...
	if (!empty($form_error_delimiters)
		...
		$constructorExtras .= "\$this->form_validation->set_error_delimiters(\"{$form_error_delimiters[0]}\", \"{$form_error_delimiters[1]}\");";
	}
}

if ($db_required != '') {
	$loadModel = "\$this->load->model('{$module_name_lower}/{$module_name_lower}_model');";
}
...
if ($controller_name_lower != $module_name_lower) {
	...
	$save_data_array = "\$data = \$this->{$module_name_lower}_model->prep_data(\$this->input->post());
